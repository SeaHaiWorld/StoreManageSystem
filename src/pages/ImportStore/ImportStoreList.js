...
const { confirm } = Modal;
@Form.create()
@connect(({ ImportStoreModel, loading, menu, tabsPanes }) => ({
  ImportStoreModel,
  loading, menu, tabsPanes,
  DataFetch: loading.effects['ImportStoreModel/fetch'],
}))

class ImportStoreList extends PureComponent {
  constructor() {
    super();
    this.state = {
      selectedRows: [],
  ...
  listPage = (params) => {
    const { dispatch, ImportStoreModel:{storeId} } = this.props;
    dispatch({
      type: "ImportStoreModel/getStoreDetail",
      payload: { id:storeId }
    });
    dispatch({
      type: 'ImportStoreModel/fetch',
  ...
  deleteStore = (record) => {
    ...
    confirm({
      ...
      onOk: () => {
        dispatch({
          type: 'ImportStoreModel/delExport',
          payload: {
              id: record.id,
          },
        });
  ...
  render() {
    ...
    const { ImportStoreModel: { data, user }, DataFetch } = this.props;
    const results = {
      list: data,
      pagination: {}
    }
    ...
    return (
      <PageHeaderWrapper title='入库管理'>
        ...
        />
        <Card bordered={false}>
          <ToolBarGroup btnOptions={this.btnList} />
          <Table
            loading={DataFetch}
            dataSource={data}
            pagination={false}
...
export default ImportStoreList;
